...
func (env *Handler) CreateClient() (*storage.Client, error) {
...
func (env *Handler) CreateBucket(client storage.Client, bucketName string) (*storage.BucketHandle, error) {
	// バケットとメタデータの設定
...
func (env *Handler) CopyDirectory(bucket storage.BucketHandle) (int, error, []error) {
	var errs []error
	objectNum := 0
...
func (env *Handler) copyFile(bucket storage.BucketHandle, file fs.DirEntry) error {
	// ローカルのファイルを開く
